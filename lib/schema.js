  ...
  validate.seal = function() {
    if (schemaObject.leafs) {
      ...
      if (leafs.uncertain.indexOf(schema.self) !== -1) {
        var assembly = validate.assembly
        ...
        // If validate is not compiled, or there are referenced external schemas then
        // schema.self need to be pointed to this schema, and then it must be reset
        if (!assembly || assembly.references.some(function(ref) { return !!ref.schema }) ) {
