...
ObjectSchema.prototype = {
  ...
  toJSON : function() {
    ...
    for (i in this.stringProps) {
      ...
      if (property.description) json.properties[property.key].description = property.description
    }
    ...
    for (i = 0; i < this.regexpProps.length; i++) {
      ...
      if (property.description) json.patternProperties[regexp].description = property.description
    }
...
schema.fromJS.def(function(object) {
  ...
  var first, value, properties = [], other, property
  for (var key in object) {
    ...
    key = key.replace(/\s*:[^:]+$/, function(match) {
      property.description = match.replace(/^\s*:\s*/, '')
      return ''
    })
...
schema.fromJSON.def(function(sch) {
  ...
  for (key in sch.properties) {
    properties.push({ min : sch.properties[key].required ? 1 : 0
                    ...
                    , description : sch.properties[key].description
                    })
  }
  for (key in sch.patternProperties) {
    properties.push({ min : 0
                    ...
                    , key : RegExp('^' + key + '$')
                    , value : schema.fromJSON(sch.properties[key])
                    , description : sch.patternProperties[key].description
                    })
  }
