...
Array.of = function() {
  ...
  // Array.of(undefined) is the same as schema(Array) so it doesn't make sense to write the former.
  // If undefined is given we interpret it as schema.self.
  var itemSchema = (args.length === 1 && args[0] === undefined) ? schema.self
                                                                : schema.fromJS(args[0])
                                                                    
  return new ArraySchema(itemSchema, args[1], args[2])
