...
var ArraySchema = module.exports = Schema.extensions.ArraySchema = Schema.extend({
  ...
  errors : function(instance) {
        console.log("ArraySchema.errors | instance = ",instance);
        var self  =this;
        var err = function(message, obj){
            value = obj || instance;
            return {
                schema:self.toJSON(),
                message:message,
        ...
        if (this.min === this.max) {
          if (instance.length !== this.min) return err("Array length should "+this.min+" and is " + instance.length)

        } else {
          if (this.min > 0        && instance.length < this.min) return err("Array length should be less than "+this.min+" and is " + instance.length);
          if (this.max < Infinity && instance.length > this.max) return err("Array length should be more than "+this.max+" and is " + instance.length);
        }
        ...
        if(resultKeysArray.length > 0){
            return err(resultKeysArray.length +" of "+instance.length + " were invalid.", results);
        }
