  ...
  errors: function(instance) {
    ...
    if (this.min === this.max) {
      if (instance.length !== this.min)
        return self.err('Array length should be equal to ' + this.min + ' and is ' + instance.length)

    } else {
      if (this.min > 0 && instance.length < this.min)
        return self.err('Array length should not be less than ' + this.min + ' and is ' + instance.length)
      if (this.max < Infinity && instance.length > this.max)
        return self.err('Array length should not be more than ' + this.max + ' and is ' + instance.length)
    }
